...
void conv_unroll(const float* in, float* out, const float* ker,
    ...
    for (int oy = 0; oy < H; ++oy) {
        for (int ox = 0; ox < W; ++ox) {
            float acc0 = 0.0f, acc1 = 0.0f, acc2 = 0.0f, acc3 = 0.0f, acc4 = 0.0f, acc5 = 0.0f, acc6 = 0.0f, acc7 = 0.0f, acc8 = 0.0f, acc9 = 0.0f, acc10 = 0.0f, acc11 = 0.0f, acc12 = 0.0f;
            for (int ky = 0; ky < K; ++ky) {
                for (kx = 0; kx < K - 12; kx+=13) {
                    ...
                    acc11 += in[(oy + ky) * in_stride + (ox + kx + 11)] * ker[ky * K + kx + 11];
                    acc12 += in[(oy + ky) * in_stride + (ox + kx + 12)] * ker[ky * K + kx + 12];
                }
                for(; kx < K; ++kx){
                    acc0 += in[(oy + ky) * in_stride + (ox + kx)] * ker[ky * K + kx];
                }
            }
            out[oy * W + ox] = acc0 + acc1 + acc2 + acc3 + acc4 + acc5 + acc6 + acc7 + acc8 + acc9 + acc10 + acc11 + acc12;
        }
    }
}
